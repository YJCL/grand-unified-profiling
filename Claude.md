...
- **ココナラPDF（商品A）**: ¥10,000・納期5日・完全個別作成。累計閲覧約7・販売0（露出不足が主因という仮説。詳細は `orba_sales_strategy_handoff.md`）
- **Orba本体サブスク（商品B）**: 目標単価 **¥3,000/月想定**（旧文書に残っていた¥550/¥500は古い数字。このチャットでの価格戦略転換＝少数高単価モデルへの移行を反映した値に統一）。決済レールは未確定（ococala／ココナラ定期購入などが候補）。
