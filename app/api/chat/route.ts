...
export async function POST(request: Request) {
    try {
        ...
        // 2) 無料ユーザーの通常上限。鑑定チケットは専用の「今日の鑑定」でのみ使う。
        if (!unlimited && usedToday >= FREE_DAILY_LIMIT) {
            return NextResponse.json({
                error: 'Daily limit reached',
                limitReached: true,
                message: `本日の無料相談回数（${FREE_DAILY_LIMIT}回）に達しました。Orba Plusなら1日${ABUSE_DAILY_CAP}回まで話せます。`
            }, { status: 429 });
        }
...
### 本格鑑定との境界
- 通常会話では、占術データを自然ににじませながら短く応える。
- 「鑑定して」「占って」と頼まれても、長い構造化鑑定へ切り替えない。本格的な読みはチャット画面の「今日の鑑定」で提供していることを、会話を遮らない一文で案内する。
- 今日・今週の流れを聞かれた場合は簡潔に答えてよいが、複数領域を網羅する長文鑑定は行わない。
